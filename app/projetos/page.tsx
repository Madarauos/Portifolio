...
const todosProjetos = [
  {
    id: "01", title: "Sistema de Presença Institucional",
  ...
  {
    id: "03", title: "Sistema de Gestão de Mocidade",
  ...
  {
    id: "02", title: "Plataforma Instituto Infância",
  ...
  {
    id: "05", title: "Agro Plants Now",
    problem: "Projeto acadêmico simulando sistema real de gestão de vendas para agroindústria.",
    role: "Desenvolvedor em equipe com Scrum — responsável por módulos de pedidos e relatórios.",
...
export default function ProjetosPage() {
  ...
  return (
    <div style={{ paddingTop: "64px", minHeight: "100vh" }}>
      ...
      {/* ── HEADER ───────────────────────────────────────────────── */}
      <section className="grid-bg" style={{ padding: "clamp(60px, 10vw, 80px) clamp(20px,5vw,48px) clamp(40px, 8vw, 60px)", borderBottom: "1px solid rgba(174,0,255,0.3)" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <div className="project-stats" style={{ display: "flex", gap: "clamp(20px, 4vw, 32px)", marginTop: 32, flexWrap: "wrap" }}>
            {[
              ...
              <div key={label}>
                <div style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 800, fontSize: "clamp(1.5rem, 4vw, 2rem)", color: "#ae00ff", lineHeight: 1 }}>{value}+</div>
                <div style={{ fontSize: "clamp(0.6rem, 1.5vw, 0.68rem)", color: "#555", textTransform: "uppercase", letterSpacing: "0.1em", marginTop: 4 }}>{label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── FILTROS ──────────────────────────────────────────────── */}
      <section style={{ padding: "20px clamp(20px,5vw,48px)", borderBottom: "1px solid rgba(255,255,255,0.04)", background: "#0d0d0d" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto", display: "flex", gap: 8, flexWrap: "wrap" }}>
          {categorias.map(cat => (
            <button key={cat} onClick={() => setActive(cat)} style={{
              ...
              fontFamily: "'DM Mono', monospace", fontSize: "clamp(0.6rem, 1.5vw, 0.7rem)",
      ...
      {/* ── GRID ─────────────────────────────────────────────────── */}
      <section style={{ padding: "56px clamp(20px,5vw,48px) 100px" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(280px, 100%), 1fr))", gap: 20 }}>
            {filtered.map((p, i) => (
              <div key={i} className="card-hover" style={{
                background: "#111", border: "1px solid rgba(255,255,255,0.06)",
                borderRadius: "8px", padding: "clamp(20px, 4vw, 28px)", position: "relative",
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 14 }}>
                  {p.filters.map(f => (
                    <span key={f} style={{ padding: "4px 10px", background: `${p.accent}15`, border: `1px solid ${p.accent}30`, borderRadius: "100px", fontSize: "0.6rem", color: p.accent, fontFamily: "'DM Mono', monospace", letterSpacing: "0.08em" }}>{f}</span>
                  ))}
                </div>

                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <span style={{ fontSize: "0.6rem", color: badgeColor[p.badge] ?? "#ae00ff", border: `1px solid ${badgeColor[p.badge] ?? "#ae00ff"}33`, padding: "3px 10px", borderRadius: "999px", fontFamily: "'DM Mono', monospace", display: "flex", alignItems: "center", gap: 6 }}>
                    {p.badge}
                ...
                <h3 style={{ fontFamily: "'Poppins', sans-serif", fontWeight: 700, fontSize: "1rem", color: "azure", marginBottom: 14 }}>{p.title}</h3>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
                  {[
                    ...
                    <div key={label} style={{ display: "flex", gap: 8 }}>
                      ...
                      <span style={{ fontSize: "0.73rem", color: "#666", lineHeight: 1.5 }}>{text}</span>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: "auto", marginBottom: 16 }}>
                  {p.tags.map(tag => (
                    <span key={tag} style={{ padding: "3px 10px", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "100px", fontSize: "0.62rem", color: "#6b6b6b", fontFamily: "'DM Mono', monospace" }}>{tag}</span>
                  ))}
                </div>
